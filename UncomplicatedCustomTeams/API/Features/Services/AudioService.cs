    ...
    public static class AudioService
    {
        ...
        private static void InitializeReflection()
        {
            ...
            try
            {
                ...
                {
                    LogManager.Warn("AudioPlayerApi assembly not found. Custom audio will not work.");
                    return;
                }
        ...
        public static void PreloadTeamAudio(Team team)
        {
            ...
            for (int i = 0; i < team.SoundPaths.Count; i++)
            {
                ...
                if (!string.IsNullOrEmpty(soundEntry.Path) && !soundEntry.Path.Contains("/path/to/your"))
                {
                    ...
                    {
                        LogManager.Warn($"Audio preload error for team {team.Name}: {e.InnerException?.Message ?? e.Message}");
                    }
                }
            }
        }
        ...
        private static IEnumerator<float> PlaySoundSequence(Team team)
        {
            ...
            {
                LogManager.Error($"Failed to create Audio Player: {ex.InnerException?.Message ?? ex.Message}");
                yield break;
            }
            ...
            if (audioPlayerInstance != null)
            {
                ...
                {
                    LogManager.Warn($"Failed to add speaker: {ex.Message}");
                }
